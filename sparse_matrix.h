...
void smat_t<val_type>::load(size_t _rows, size_t _cols, size_t _nnz, const char* filename, smat_t<val_type>::format_t fmt){
	//printf("***************************************\n");
    //printf("parameter in smat_t load() function is %lu %lu %lu %s %lu \n", _rows, _cols, _nnz, filename, fmt);
...
void smat_t<val_type>::load_from_PETSc(const char *filename) {
	...
	//printf("filetyple:%lu   rows:%lu    cols:%lu \n",int_buf[0],int_buf[1],int_buf[2]);
	int filetype = int_buf[0]; //what is 1015
	...
	mem_alloc_by_me = true;
	//printf("nzz in smat_t load_from_PETSc is %lu \n",nnz);
	val = MALLOC(val_type, nnz); val_t = MALLOC(val_type, nnz);
	row_idx = MALLOC(unsigned, nnz); col_idx = MALLOC(unsigned, nnz);
	row_ptr = MALLOC(size_t, rows+1); col_ptr = MALLOC(size_t, cols+1);
	...
	{ 
		...
		//printf("nnz_row[0] in smat_t load_from_PETSc is %lu \n",nnz_row[0]);
		//printf("nnz_row[1] in smat_t load_from_PETSc is %lu \n",nnz_row[1]);
		//printf("nnz_row[2] in smat_t load_from_PETSc is %lu \n",nnz_row[2]);
		...
		// read col_idx 
		    //printf("row_ptr[%lu] in smat_t load_from_PETSc is %lu \n",r,row_ptr[r]);
		}
		headersize += sizeof(int)*fread(&col_idx[0], sizeof(unsigned), nnz, fp);
		//printf("col_idx[0] in smat_t load_from_PETSc is %lu \n",col_idx[0]);
		//printf("col_idx[1] in smat_t load_from_PETSc is %lu \n",col_idx[1]);
		//printf("col_idx[2] in smat_t load_from_PETSc is %lu \n",col_idx[2]);
		//printf("col_idx[34699] in smat_t load_from_PETSc is %lu \n",col_idx[34699]);
		//printf("row_ptr[0] in smat_t load_from_PETSc is %lu \n",row_ptr[0]);
		//printf("row_ptr[1] in smat_t load_from_PETSc is %lu \n",row_ptr[1]);
		//printf("row_ptr[2] in smat_t load_from_PETSc is %lu \n",row_ptr[2]);
		// read val_t 
		const size_t chunksize = 1024;
		double buf[chunksize];
		size_t idx = 0;
		...
		//printf("val_t[0] in smat_t load_from_PETSc is %lu \n",val_t[0]);
		//printf("val_t[1] in smat_t load_from_PETSc is %lu \n",val_t[1]);
		//printf("val_t[2] in smat_t load_from_PETSc is %lu \n",val_t[2]);
		//printf("val_t[13] in smat_t load_from_PETSc is %lu \n",val_t[13]);
	}
	fclose(fp);
	...
	//printf("val[0] in smat_t load_from_PETSc is %lu \n",val[0]);
	//printf("val[1] in smat_t load_from_PETSc is %lu \n",val[1]);
	//printf("val[2] in smat_t load_from_PETSc is %lu \n",val[2]);
	//printf("val[13] in smat_t load_from_PETSc is %lu \n",val[13]);
	//printf("col_ptr[0] in smat_t load_from_PETSc is %lu \n",col_ptr[0]);
	//printf("col_ptr[1] in smat_t load_from_PETSc is %lu \n",col_ptr[1]);
	//printf("col_ptr[2] in smat_t load_from_PETSc is %lu \n",col_ptr[2]);
	//printf("col_ptr[3] in smat_t load_from_PETSc is %lu \n",col_ptr[3]);	
	//printf("col_ptr[12418] in smat_t load_from_PETSc is %lu \n",col_ptr[12418]);
	//printf("col_ptr[12419] in smat_t load_from_PETSc is %lu \n",col_ptr[12419]);
	//printf("row_idx[0] in smat_t load_from_PETSc is %lu \n",row_idx[0]);
	//printf("row_idx[1] in smat_t load_from_PETSc is %lu \n",row_idx[1]);
	//printf("row_idx[2] in smat_t load_from_PETSc is %lu \n",row_idx[2]);
	max_row_nnz = max_col_nnz = 0;
	...
	//printf("%lu \n", nnz_of_row(0));
	//printf("%lu \n", nnz_of_row(1));
	//printf("max_col_nnz:%lu max_row_nnz:%lu\n",max_col_nnz,max_row_nnz);
...
void smat_t<val_type>::csr_to_csc() {
	...
	for(size_t r = 0; r < rows; r++) {
		for(size_t idx = row_ptr[r]; idx != row_ptr[r+1]; idx++) {
			...
			/*if(r==67)
			{
				printf("val[%lu] in smat_t load_from_PETSc is c:%lu , val_t[idx]:%lu col_ptr[%lu]:%lu     \n",r,c,val_t[idx],c,col_ptr[c]++);
			}*/
		}
	}
	...
	//for(size_t r = 0; r < rows; r++) {
	//	printf("val[%lu] in smat_t load_from_PETSc is %lu \n",r,val[r]);
	//}
